...
for pressure in tqdm(pressures, desc=' pressure', position=0):
    for start_voltage, end_voltage in v_pairs:
        ...
        voltage = (start_voltage + end_voltage)/2
        ...
        for parameter in parameters:
            ...
            interpolator = interp1d([start_voltage, end_voltage], np.vstack([start_array, end_array]), axis=0)
            inter_array = pd.Series(interpolator(voltage), name=parameter)
            parameter_arrays.append(inter_array)
        ...
        inter_df[list(start_df.columns)].to_feather(feather_outdir / f'{voltage:.0f}Vpp{pressure:03.0f}Pa_node.feather')
...
for voltage in tqdm(voltages, desc='voltage'):
    for start_pressure, end_pressure in p_pairs:
        ...
        parameters = list(start_df.columns[4:])
        pressure = (start_pressure + end_pressure)/2
        ...
        for parameter in parameters:
            start_array = start_df[parameter].values
            end_array = end_df[parameter].values
            interpolator = interp1d([start_pressure, end_pressure], np.vstack([start_array, end_array]), axis=0)
            inter_array = pd.Series(interpolator(pressure), name=parameter)
            parameter_arrays.append(inter_array)
